...
var xpFlag = 0;//0 = xp spending off 1 = normal xp spending 2 = pracical xp spending
...
if (localStorage.getItem("saveList") === null) {
    localStorage.setItem("saveList", ",");    //Init the save list if no files have been saved to local storage
}
saveList = localStorage.getItem("saveList").split(",");//populates the list of saves
var player = {//inits a new player at start of program
    "strength" : 1,
    "dexterity" : 1,
    "stamina" : 1,
    "inteligence" : 1,
...
/*
 * JSON object containing all merit information
 */
var merits = [
        {
...
/*
 * JSON object containing all vamiric dicipline information
 */
var disciplines = [
        {
            "name": "Celerity•",
...
/*
 * JSON object containing all vampiric devotion information
 */
var devotions = [
        {
            "name": "Arcane Sight ",
...
function loadVampire() {//Gens the vampire supernatural tab
    ...
    for (i = 0; i < player.disciplineList.length; i++) {
    ...
    for (i = 0; i < player.devotionList.length; i++) {
...
function loadWerewolf() {//Gens werewolf supernatural tab
    //TODO
}
function loadMage() {//Gens mage supernatural tab
    //TODO
}
function loadChangeling() {//Gens Changeling supernatural tab
    //TODO
}
function loadGeist() {//Gens Geist supernatural tab
    //TODO
}
function loadPromethean() {//Gens Promethean supernatural tab
    //TODO
}
/*
...
function addNormalXP() {//adds to the players normal xp
    player.normalXP += parseInt(document.getElementById("normalXPInput").value, 10);
    loadOtherTab();
}
function addOtherXP() {//adds to the players practical xp
    player.otherXP += parseInt(document.getElementById("otherXPInput").value, 10);
    loadOtherTab();
}
function addSpendXP() {//spends xp based on the xp flag
    if (xpFlag === 1) {
        player.spentNormalXP += parseInt(document.getElementById("spentXPInput").value, 10);
    }
    else if (xpFlag === 2) {
        player.spentOtherXP += parseInt(document.getElementById("spentXPInput").value, 10);
    }
    loadOtherTab();
}
/*
 * saves a player to local storage and adds them to the saveList so they will show up on future runs of the code
 */
function savePlayer() {
    var saveFlag = 0;
...
function loadPlayer() {//loads a player from local storage
    player = JSON.parse(localStorage.getItem(saveList[selectedSave]));
}
function deletePlayer() {//removes a player from the save list
  saveList.splice(selectedSave,1);
  localStorage.setItem("saveList",saveList.join(","));
  loadSaveTab();
}
function genAtrTab() {//Creates the attributes tab
...
function genSkillsTab() {//Creates the skills tab
...
function genMeritsTab () {//Creates the Merits tab
...
function genOtherTab() {//Creates the other information tab
...
function addSpecialty () {//Adds a specalty to the players list of specalties
    if (xpFlag === 1) {
...
function removeSpecialty () {//Removes a specialty from the players list of specialtys
    if (xpFlag === 1) {
        player.spentNormalXP += -3;
    }
    else if (xpFlag === 2) {
...
function selectMorality(morality) {//Changes players morality score
    player.morality = morality;
    loadOtherTab();
}
function displayMeritDescription (meritIndex) {//displays the description of the selected Merit in the all mertis section
    meritsDesciptionFlag = meritIndex;
    loadMeritsTab();
}
function displayMeritDescriptionOwned (meritIndex) {//displays the description of the selected merit in the owned merits section
    meritsDesciptionFlagOwned = meritIndex;
    loadMeritsTab();
}
function displayPowerDescription (powerIndex) {//displayes the decription of the selected supernatural power in the all powers section
    powerDesciptionFlag = powerIndex;
    loadSuperNatural();
}
function displayPowerDescriptionOwned (powerIndex) {//displays the description of the selected supernatural power in the owned powers section
    powerDesciptionFlagOwned = powerIndex;
    loadSuperNatural();
}
function displayDisciplineDescription (powerIndex) {//displays the description of the selected vampiric disipline in the all disipline section
    disciplineDesciptionFlag = powerIndex;
    loadSuperNatural();
}
function displayDisciplineDescriptionOwned (powerIndex) {//displays the description of the selected vampiric disipline in the owned disipline section
    disciplineDesciptionFlagOwned = powerIndex;
    loadSuperNatural();
}
function displayDevotionDescription (powerIndex) {//displays the description of the selected vampiric devotion in the all devotion section
    devotionDesciptionFlag = powerIndex;
    loadSuperNatural();
}
function displayDevotionDescriptionOwned (powerIndex) {//displays the description of the selected vampiric devotion in the owned devotion section
    devotionDesciptionFlagOwned = powerIndex;
    loadSuperNatural();
}
function gainMerit () {//adds a merit to players list of merits and spends xp based on xp flag
...
function gainDiscipline () {//adds a discipline to players list of discipline and spends xp based on xp flag
...
function gainDevotion () {//adds a devotion to players list of devotions and spends xp based on xp flag
      var selected = document.getElementById("devotionSelected");
      var index = parseInt(selected.className);
      player.devotionList.push(devotions[index]);
      loadSuperNatural();
}
function removeMerit () {//removes merit and  unspends corisponding xp
...
function removeDiscipline () {//removes discipline and  unspends corisponding xp
...
function removeDevotion () {//removes devotion and  unspends corisponding xp
...
function spendAtrXP (oldRating,newRating) {//spends or unspends xp when an atribute value is altered
...
function spendTemplateXP (oldRating,newRating) {//spends or unspends xp when template value is altered
      if (newRating > oldRating) {
          if (xpFlag === 1) {
              for(i = oldRating;i < newRating; i++) {
...
function spendSkillXP (oldRating,newRating) {//spends or unspends xp when skill value is altered
      if (newRating > oldRating) {
          if (xpFlag === 1) {
              for(i = oldRating;i < newRating; i++) {
...
function setTemplateValue (value) {//changes supernatural template value
    spendTemplateXP(player.templateValue,value);
      player.templateValue = value;
      loadSuperNatural();
}
/*
 * the flowing are setters for the players atribute and skill values
 */
